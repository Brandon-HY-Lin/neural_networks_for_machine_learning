...
def func_mesh(ax):
    ...
    ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, alpha=0.3,
                                ...
                                cstride=5, rstride=5, edgecolor='dimgrey')
...
### momentum < 1
    print "start training with momentum < 1" 
    ...
    training.start()
    history_momentum_less_1 = training.get_history()

### momentum > 1
    lr = 0.0001
    momentum = 1.005
    print "start training with momentum = ", momentum

    w = theano.shared(w_init, 'w')
    optimizer = SGD(cost_func, w, lr, reg, momentum=momentum) 
    training = TrainProcess(optimizer, max_iter, print_period)
    training.start()
    history_momentum_greater_1 = training.get_history()
...
### momentum > 1.05
    ...
    training = TrainProcess(optimizer, max_iter, print_period)
    training.start()
    history_momentum_max = training.get_history()
    ...
    # prepare data
    data_momentum_less_1 = swapaxes(history_momentum_less_1)
    data_momentum_greater_1 = swapaxes(history_momentum_greater_1)
    data_momentum_max = swapaxes(history_momentum_max)

    data = np.array([
        data_momentum_less_1, data_momentum_greater_1, data_momentum_max
        ])

    labels = ['Momentum < 1', 'Momentum = 1.005', 'Momentum = 1.05']
